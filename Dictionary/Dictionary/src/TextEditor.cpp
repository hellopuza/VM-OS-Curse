...
void TextEditor::replace(iterator& it, const std::string& str)
{
    file_content_.replace(it.current_position(), it.current_word().length(), str);
    it = iterator(it.current_position(), this);
}
    ...
    begin_pos_(position), text_ed_(text_ed)
{
    ...
    while ((end_word_pos_ < text_ed_->file_content_.length()) && !isspace(text_ed_->file_content_[end_word_pos_]))
    {
        end_word_pos_++;
    }
    while ((end_word_pos_ > 0) && !isalpha(text_ed_->file_content_[end_word_pos_]))
    {
        end_word_pos_--;
    }
...
TextEditor::iterator& TextEditor::iterator::next()
{
